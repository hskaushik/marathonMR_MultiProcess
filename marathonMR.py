...
if __name__ == "__main__":
    ...
    try :
        cmd = "`%s/marathonMR.sh %s %s %s %s %s %s >/dev/null 2>&1` & "%(basePath,basePath,work_dir,diffraction_dataset,pdb_path,jobs_in_parallel,time_out)
        #cmd = "`%s/marathonMR.sh %s %s %s %s %s %s` "%(basePath,basePath,work_dir,diffraction_dataset,pdb_path,jobs_in_parallel,time_out)
        #print cmd
